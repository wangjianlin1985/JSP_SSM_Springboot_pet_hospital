...
public class OrderInfoController extends BaseController {
	...
	@InitBinder("expertObj")
	public void initBinderexpertObj(WebDataBinder binder) {
		binder.setFieldDefaultPrefix("expertObj.");
	}
	@InitBinder("userObj")
	...
	public void list(@ModelAttribute("expertObj") Expert expertObj,String orderDate,@ModelAttribute("userObj") UserInfo userObj,String handleState,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (orderDate == null) orderDate = "";
		if (handleState == null) handleState = "";
		if(rows != 0)orderInfoService.setRows(rows);
		List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(expertObj, orderDate, userObj, handleState, page);
	    /*计算总的页数和总的记录数*/
	    orderInfoService.queryTotalPageAndRecordNumber(expertObj, orderDate, userObj, handleState);
	    /*获取到总的页码数目*/
	...
	/*ajax方式按照查询条件分页查询订单信息*/
	@RequestMapping(value = { "/expertlist" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void expertlist(@ModelAttribute("expertObj") Expert expertObj,String orderDate,@ModelAttribute("userObj") UserInfo userObj,String handleState,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response,HttpSession session) throws Exception {
		if (page==null || page == 0) page = 1;
		if (orderDate == null) orderDate = "";
		if (handleState == null) handleState = "";
		if(rows != 0)orderInfoService.setRows(rows);
		...
		expertObj.setExpertUserName(session.getAttribute("username").toString()); 
	...
	public String frontlist(@ModelAttribute("expertObj") Expert expertObj,String orderDate,@ModelAttribute("userObj") UserInfo userObj,String handleState,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (orderDate == null) orderDate = "";
		if (handleState == null) handleState = "";
		List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(expertObj, orderDate, userObj, handleState, currentPage);
	    /*计算总的页数和总的记录数*/
	    orderInfoService.queryTotalPageAndRecordNumber(expertObj, orderDate, userObj, handleState);
	    /*获取到总的页码数目*/
	    int totalPage = orderInfoService.getTotalPage();
	    ...
	    request.setAttribute("expertObj", expertObj);
	...
	public String userFrontlist(@ModelAttribute("expertObj") Expert expertObj,String orderDate,@ModelAttribute("userObj") UserInfo userObj,String handleState,Integer currentPage, Model model, HttpServletRequest request, HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (orderDate == null) orderDate = "";
		if (handleState == null) handleState = "";
	...
	public void OutToExcel(@ModelAttribute("expertObj") Expert expertObj,String orderDate,@ModelAttribute("userObj") UserInfo userObj,String handleState, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(orderDate == null) orderDate = "";
        if(handleState == null) handleState = "";
        List<OrderInfo> orderInfoList = orderInfoService.queryOrderInfo(expertObj,orderDate,userObj,handleState);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "OrderInfo信息记录"; 
        String[] headers = { "预约id","预约专家","预约日期","预约时间","预约用户","处理状态"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<orderInfoList.size();i++) {
        	OrderInfo orderInfo = orderInfoList.get(i); 
        	dataset.add(new String[]{orderInfo.getOrderId() + "",orderInfo.getExpertObj().getName(),orderInfo.getOrderDate(),orderInfo.getOrderTime(),orderInfo.getUserObj().getNicheng(),orderInfo.getHandleState()});
        }
        /*
