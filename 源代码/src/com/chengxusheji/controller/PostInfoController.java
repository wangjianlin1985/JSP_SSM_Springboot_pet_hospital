...
public class PostInfoController extends BaseController {
	...
	public void list(String title,@ModelAttribute("petClassObj") PetClass petClassObj,@ModelAttribute("userObj") UserInfo userObj,String addTime,String jinghuaFlag,String shenHeState,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (title == null) title = "";
		if (addTime == null) addTime = "";
		if (jinghuaFlag == null) jinghuaFlag = "";
		if (shenHeState == null) shenHeState = "";
		if(rows != 0)postInfoService.setRows(rows);
		List<PostInfo> postInfoList = postInfoService.queryPostInfo(title, petClassObj, userObj, addTime, jinghuaFlag, shenHeState, page);
	    /*计算总的页数和总的记录数*/
	    postInfoService.queryTotalPageAndRecordNumber(title, petClassObj, userObj, addTime, jinghuaFlag, shenHeState);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String title,@ModelAttribute("petClassObj") PetClass petClassObj,@ModelAttribute("userObj") UserInfo userObj,String addTime,String jinghuaFlag,String shenHeState,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (title == null) title = "";
		if (addTime == null) addTime = "";
		if (jinghuaFlag == null) jinghuaFlag = "";
		shenHeState = "已审核";
		List<PostInfo> postInfoList = postInfoService.queryPostInfo(title, petClassObj, userObj, addTime, jinghuaFlag, shenHeState, currentPage);
	    /*计算总的页数和总的记录数*/
	    postInfoService.queryTotalPageAndRecordNumber(title, petClassObj, userObj, addTime, jinghuaFlag,shenHeState);
	    /*获取到总的页码数目*/
	    int totalPage = postInfoService.getTotalPage();
	    ...
	    request.setAttribute("jinghuaFlag", jinghuaFlag);
	    List<PetClass> petClassList = petClassService.queryAllPetClass();
	    request.setAttribute("petClassList", petClassList);
	    List<UserInfo> userInfoList = userInfoService.queryAllUserInfo();
	    request.setAttribute("userInfoList", userInfoList);
	...
	public String userFrontlist(String title,@ModelAttribute("petClassObj") PetClass petClassObj,@ModelAttribute("userObj") UserInfo userObj,String addTime,String jinghuaFlag, String shenHeState,Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (title == null) title = "";
		if (addTime == null) addTime = "";
		if (jinghuaFlag == null) jinghuaFlag = "";
	...
	public void OutToExcel(String title,@ModelAttribute("petClassObj") PetClass petClassObj,@ModelAttribute("userObj") UserInfo userObj,String addTime,String jinghuaFlag, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(title == null) title = "";
        if(addTime == null) addTime = "";
        if(jinghuaFlag == null) jinghuaFlag = "";
        List<PostInfo> postInfoList = postInfoService.queryPostInfo(title,petClassObj,userObj,addTime,jinghuaFlag);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "PostInfo信息记录"; 
        String[] headers = { "帖子id","帖子标题","宠物分类","浏览量","发帖人","发帖时间","是否精华帖"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<postInfoList.size();i++) {
        	PostInfo postInfo = postInfoList.get(i); 
        	dataset.add(new String[]{postInfo.getPostInfoId() + "",postInfo.getTitle(),postInfo.getPetClassObj().getPetClassName(),postInfo.getHitNum() + "",postInfo.getUserObj().getNicheng(),postInfo.getAddTime(),postInfo.getJinghuaFlag()});
        }
        /*
