...
public class LeavewordController extends BaseController {
	...
	/*ajax方式按照查询条件分页查询用户咨询信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(String leaveTitle,@ModelAttribute("userObj") UserInfo userObj,String leaveTime, String shenHeState, Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<Leaveword> leavewordList = leavewordService.queryLeaveword(leaveTitle, userObj, leaveTime, shenHeState, page);
	    /*计算总的页数和总的记录数*/
	    leavewordService.queryTotalPageAndRecordNumber(leaveTitle, userObj, leaveTime, shenHeState);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String leaveTitle,@ModelAttribute("userObj") UserInfo userObj,String leaveTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (leaveTitle == null) leaveTitle = "";
		if (leaveTime == null) leaveTime = "";
		List<Leaveword> leavewordList = leavewordService.queryLeaveword(leaveTitle, userObj, leaveTime, "已审核", currentPage);
	    /*计算总的页数和总的记录数*/
	    leavewordService.queryTotalPageAndRecordNumber(leaveTitle, userObj, leaveTime, "已审核");
	    /*获取到总的页码数目*/
	    int totalPage = leavewordService.getTotalPage();
	    /*当前查询条件下总记录数*/
	    int recordNumber = leavewordService.getRecordNumber();
	...
	public String userFrontlist(String leaveTitle,@ModelAttribute("userObj") UserInfo userObj,String leaveTime,String shenHeState, Integer currentPage, Model model, HttpServletRequest request,HttpSession session) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (leaveTitle == null) leaveTitle = "";
		if (leaveTime == null) leaveTime = "";
		if (shenHeState == null) shenHeState = "";
		...
		List<Leaveword> leavewordList = leavewordService.queryLeaveword(leaveTitle, userObj, leaveTime, shenHeState, currentPage);
	    /*计算总的页数和总的记录数*/
	    leavewordService.queryTotalPageAndRecordNumber(leaveTitle, userObj, leaveTime, shenHeState);
	    /*获取到总的页码数目*/
	    int totalPage = leavewordService.getTotalPage();
	...
	public String delete(@PathVariable Integer leaveWordId,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  leavewordService.deleteLeaveword(leaveWordId);
	            request.setAttribute("message", "用户咨询删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "用户咨询删除失败!");
				return "error";
	...
	public void OutToExcel(String leaveTitle,@ModelAttribute("userObj") UserInfo userObj,String leaveTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        ...
        String[] headers = { "留言id","留言标题","留言人","留言时间","回复内容","回复时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<leavewordList.size();i++) {
        	Leaveword leaveword = leavewordList.get(i); 
        	dataset.add(new String[]{leaveword.getLeaveWordId() + "",leaveword.getLeaveTitle(),leaveword.getUserObj().getNicheng(),leaveword.getLeaveTime(),leaveword.getReplyContent(),leaveword.getReplyTime()});
        }
        /*
