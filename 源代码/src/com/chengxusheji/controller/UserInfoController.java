...
public class UserInfoController extends BaseController {
	...
	public void add(@Validated UserInfo userInfo, BindingResult br,
		...
		try {
			userInfo.setPetPhoto(this.handlePhotoUpload(request, "petPhotoFile"));
	...
	public void list(String user_name,@ModelAttribute("petClassObj") PetClass petClassObj,String nicheng,String birthDate,String sfym,String name,String telephone,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, petClassObj, nicheng, birthDate, sfym, name, telephone, page);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, petClassObj, nicheng, birthDate, sfym, name, telephone);
	    /*获取到总的页码数目*/
	...
	public void listAll(HttpServletResponse response) throws Exception {
		...
		for(UserInfo userInfo:userInfoList) {
			...
			jsonUserInfo.accumulate("nicheng", userInfo.getNicheng());
			jsonArray.put(jsonUserInfo);
		}
	...
	public String frontlist(String user_name,@ModelAttribute("petClassObj") PetClass petClassObj,String nicheng,String birthDate,String sfym,String name,String telephone,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (name == null) name = "";
		if (telephone == null) telephone = "";
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, petClassObj, nicheng, birthDate, sfym, name, telephone, currentPage);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, petClassObj, nicheng, birthDate, sfym, name, telephone);
	    /*获取到总的页码数目*/
	    int totalPage = userInfoService.getTotalPage();
	    ...
	    request.setAttribute("sfym", sfym);
	...
	public void update(@Validated UserInfo userInfo, BindingResult br,
		...
		if(!petPhotoFileName.equals("upload/NoImage.jpg"))userInfo.setPetPhoto(petPhotoFileName); 
	...
	public void OutToExcel(String user_name,@ModelAttribute("petClassObj") PetClass petClassObj,String nicheng,String birthDate,String sfym,String name,String telephone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(user_name == null) user_name = "";
        if(nicheng == null) nicheng = "";
        if(birthDate == null) birthDate = "";
        if(sfym == null) sfym = "";
        if(name == null) name = "";
        if(telephone == null) telephone = "";
        List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name,petClassObj,nicheng,birthDate,sfym,name,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "UserInfo信息记录"; 
        String[] headers = { "用户名","宠物类别","宠物昵称","宠物照片","性别","体型","体重","出生日期","是否接种疫苗","主人姓名","联系电话","邮件","注册时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<userInfoList.size();i++) {
        	UserInfo userInfo = userInfoList.get(i); 
        	dataset.add(new String[]{userInfo.getUser_name(),userInfo.getPetClassObj().getPetClassName(),userInfo.getNicheng(),userInfo.getPetPhoto(),userInfo.getGender(),userInfo.getTixing(),userInfo.getWeight(),userInfo.getBirthDate(),userInfo.getSfym(),userInfo.getName(),userInfo.getTelephone(),userInfo.getEmail(),userInfo.getRegTime()});
        }
        /*
