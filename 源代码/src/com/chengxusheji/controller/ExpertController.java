...
public class ExpertController extends BaseController {
	...
	/*ajax方式按照查询条件分页查询宠物专家信息*/
	@RequestMapping(value = { "/list" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void list(String expertUserName,String name,String birthDate,String school,String telephone,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<Expert> expertList = expertService.queryExpert(expertUserName, name, birthDate, school, telephone, page);
	    /*计算总的页数和总的记录数*/
	    expertService.queryTotalPageAndRecordNumber(expertUserName, name, birthDate, school, telephone);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String expertUserName,String name,String birthDate,String school,String telephone,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (school == null) school = "";
		if (telephone == null) telephone = "";
		List<Expert> expertList = expertService.queryExpert(expertUserName, name, birthDate, school, telephone, currentPage);
	    /*计算总的页数和总的记录数*/
	    expertService.queryTotalPageAndRecordNumber(expertUserName, name, birthDate, school, telephone);
	    /*获取到总的页码数目*/
	    int totalPage = expertService.getTotalPage();
	...
	public String delete(@PathVariable String expertUserName,HttpServletRequest request) throws UnsupportedEncodingException {
		  try {
			  expertService.deleteExpert(expertUserName);
	            request.setAttribute("message", "宠物专家删除成功!");
	            return "message";
	        } catch (Exception e) { 
	            e.printStackTrace();
	            request.setAttribute("error", "宠物专家删除失败!");
				return "error";
	...
	/*按照查询条件导出宠物专家信息到Excel*/
	@RequestMapping(value = { "/OutToExcel" }, method = {RequestMethod.GET,RequestMethod.POST})
	public void OutToExcel(String expertUserName,String name,String birthDate,String school,String telephone, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(expertUserName == null) expertUserName = "";
        if(name == null) name = "";
        if(birthDate == null) birthDate = "";
        if(school == null) school = "";
        if(telephone == null) telephone = "";
        List<Expert> expertList = expertService.queryExpert(expertUserName,name,birthDate,school,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Expert信息记录"; 
        String[] headers = { "专家账号","姓名","性别","出生日期","专家照片","工作经验","联系电话"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<expertList.size();i++) {
        	Expert expert = expertList.get(i); 
        	dataset.add(new String[]{expert.getExpertUserName(),expert.getName(),expert.getGender(),expert.getBirthDate(),expert.getExpertPhoto(),expert.getWorkExperience(),expert.getTelephone()});
        }
        /*
