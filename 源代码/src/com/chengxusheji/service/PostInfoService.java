...
public class PostInfoService {
    ...
    /*按照查询条件分页查询帖子记录*/
    public ArrayList<PostInfo> queryPostInfo(String title,PetClass petClassObj,UserInfo userObj,String addTime,String jinghuaFlag,String shenHeState,int currentPage) throws Exception { 
     	String where = "where 1=1";
    	if(!title.equals("")) where = where + " and t_postInfo.title like '%" + title + "%'";
    	if(null != petClassObj && petClassObj.getPetClassId()!= null && petClassObj.getPetClassId()!= 0)  where += " and t_postInfo.petClassObj=" + petClassObj.getPetClassId();
    	...
    	if(!jinghuaFlag.equals("")) where = where + " and t_postInfo.jinghuaFlag like '%" + jinghuaFlag + "%'";
    	if(!shenHeState.equals("")) where = where + " and t_postInfo.shenHeState = '" + shenHeState + "'";
    	int startIndex = (currentPage-1) * this.rows;
    	return postInfoMapper.queryPostInfo(where, startIndex, this.rows);
    }

    /*按照查询条件查询所有记录*/
    public ArrayList<PostInfo> queryPostInfo(String title,PetClass petClassObj,UserInfo userObj,String addTime,String jinghuaFlag) throws Exception  { 
     	String where = "where 1=1";
    	if(!title.equals("")) where = where + " and t_postInfo.title like '%" + title + "%'";
    	if(null != petClassObj && petClassObj.getPetClassId()!= null && petClassObj.getPetClassId()!= 0)  where += " and t_postInfo.petClassObj=" + petClassObj.getPetClassId();
    	if(null != userObj &&  userObj.getUser_name() != null && !userObj.getUser_name().equals(""))  where += " and t_postInfo.userObj='" + userObj.getUser_name() + "'";
    ...
    /*当前查询条件下计算总的页数和记录数*/
    public void queryTotalPageAndRecordNumber(String title,PetClass petClassObj,UserInfo userObj,String addTime,String jinghuaFlag,String shenHeState) throws Exception {
     	String where = "where 1=1";
    	if(!title.equals("")) where = where + " and t_postInfo.title like '%" + title + "%'";
    	if(null != petClassObj && petClassObj.getPetClassId()!= null && petClassObj.getPetClassId()!= 0)  where += " and t_postInfo.petClassObj=" + petClassObj.getPetClassId();
    	if(null != userObj &&  userObj.getUser_name() != null && !userObj.getUser_name().equals(""))  where += " and t_postInfo.userObj='" + userObj.getUser_name() + "'";
