...
function initPostInfoManageTool() {
	postInfo_manage_tool = {
		init: function() {
			...
			$.ajax({
				...
				success : function (data, response, status) {
					...
					data.splice(0,0,{user_name:"",nicheng:"不限制"});
		...
		search: function() {
			...
			queryParams["jinghuaFlag"] = $("#jinghuaFlag").val();
				...
				$.ajax({
					...
					success : function (postInfo, response, status) {
						...
						if (postInfo) { 
							...
							$("#postInfo_petClassObj_petClassId_edit").combobox({
								url:"PetClass/listAll",
							    valueField:"petClassId",
							    textField:"petClassName",
							    panelHeight: "auto",
						        ...
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#postInfo_petClassObj_petClassId_edit").combobox("select", postInfo.petClassObjPri);
									//var data = $("#postInfo_petClassObj_petClassId_edit").combobox("getData"); 
						            //if (data.length > 0) {
						                //$("#postInfo_petClassObj_petClassId_edit").combobox("select", data[0].petClassId);
						            //}
								}
							});
							...
							$("#postInfo_jinghuaFlag_edit").validatebox({
								required : true,
								missingMessage : "请输入是否精华帖",
							});
