...
<%
    ...
    String jinghuaFlag = (String)request.getAttribute("jinghuaFlag"); //是否精华帖查询关键字
%>
<!DOCTYPE html>
<html>
<head>
...
<jsp:include page="../header.jsp"></jsp:include>
				    		...
				    		<div class="row">
				    			<div class="col-md-12 top5">
				    				...
				    				<table class="table table-condensed table-hover">
				    					<tr class="success bold"><td>序号</td><td>帖子标题</td><td>宠物分类</td><td>浏览量</td><td>发帖时间</td><td>是否精华帖</td><td>审核状态<td>操作</td></tr>
				    					<% 
				    						/*计算起始序号*/
 										...
 										<tr>
 											...
 											<td><%=postInfo.getPetClassObj().getPetClassName() %></td>
 											<td><%=postInfo.getHitNum() %></td>
 											<td><%=postInfo.getAddTime() %></td>
 											<td><%=postInfo.getJinghuaFlag() %></td>
 											<td><%=postInfo.getShenHeState() %></td>
 											<td>
 												<a href="<%=basePath  %>PostInfo/<%=postInfo.getPostInfoId() %>/frontshow"><i class="fa fa-info"></i>&nbsp;查看</a>&nbsp;
 												<% if (postInfo.getShenHeState().equals("待审核")) { %>
 												<a href="#" onclick="postInfoEdit('<%=postInfo.getPostInfoId() %>');"><i class="fa fa-pencil fa-fw"></i>编辑</a>&nbsp;
 												<% } %>
 												<a href="#" onclick="postInfoDelete('<%=postInfo.getPostInfoId() %>');"><i class="fa fa-trash-o fa-fw"></i>删除</a>
 											</td> 
 										</tr>
 										<%}%>
				    				</table>
	...
	<div class="col-md-3 wow fadeInRight">
		...
		<form name="postInfoQueryForm" id="postInfoQueryForm" action="<%=basePath %>PostInfo/userFrontlist" class="mar_t15" method="post">
                ...
                <select id="userObj_user_name" name="userObj.user_name" class="form-control">
                	<option value="">不限制</option>
 					...
 					if(userObj!=null && userObj.getUser_name()!=null && userObj.getUser_name().equals(userInfoTemp.getUser_name()))
 						selected = "selected";
	 				%>
 				 <option value="<%=userInfoTemp.getUser_name() %>" <%=selected %>><%=userInfoTemp.getNicheng() %></option>
	 				<%
	 				}
	 				%>
 			</select>
			...
			<div class="form-group">
				<label for="jinghuaFlag">是否精华帖:</label>
				<input type="text" id="jinghuaFlag" name="jinghuaFlag" value="<%=jinghuaFlag %>" class="form-control" placeholder="请输入是否精华帖">
			</div>
  ...
  <div class="modal-dialog" style="width:900px;" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" style="height:450px; overflow: scroll;">
		  ...
		  <div class="form-group">
		  	 <label for="postInfo_petClassObj_petClassId_edit" class="col-md-3 text-right">宠物分类:</label>
		  	 <div class="col-md-9">
			    <select id="postInfo_petClassObj_petClassId_edit" name="postInfo.petClassObj.petClassId" class="form-control">
			    </select>
		  	 </div>
		  </div>
		  <div class="form-group">
		  ...
		  <div class="form-group" style="display:none;">
		  	 <label for="postInfo_jinghuaFlag_edit" class="col-md-3 text-right">是否精华帖:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="postInfo_jinghuaFlag_edit" name="postInfo.jinghuaFlag" class="form-control" placeholder="请输入是否精华帖">
			 </div>
		  </div>
		  <div class="form-group" style="display:none;">
		     <label for="postInfo_shenHeState_edit" class="col-md-3 text-right">审核状态:</label>
		  	 <div class="col-md-9">
			    <input type="text" id="postInfo_shenHeState_edit" name="postInfo.shenHeState" class="form-control" placeholder="审核状态">
			 </div>
		  </div>
		</form> 
			...
			if (postInfo) {
				...
				$.ajax({
					...
					success: function(petClasss,response,status) { 
						$("#postInfo_petClassObj_petClassId_edit").empty();
						var html="";
		        		$(petClasss).each(function(i,petClass){
		        			html += "<option value='" + petClass.petClassId + "'>" + petClass.petClassName + "</option>";
		        		});
		        		$("#postInfo_petClassObj_petClassId_edit").html(html);
		        		$("#postInfo_petClassObj_petClassId_edit").val(postInfo.petClassObjPri);
					}
				});
				...
				$.ajax({
					...
					success: function(userInfos,response,status) { 
		        		...
		        		$(userInfos).each(function(i,userInfo){
		        			html += "<option value='" + userInfo.user_name + "'>" + userInfo.nicheng + "</option>";
		        		});
				...
				$("#postInfo_jinghuaFlag_edit").val(postInfo.jinghuaFlag);
				$("#postInfo_shenHeState_edit").val(postInfo.shenHeState);
